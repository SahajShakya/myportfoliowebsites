<?php
function handleAcademicProjectsRoutes($method, $segments, $db) {
    require_once __DIR__ . '/../models/AcademicProject.php';
    require_once __DIR__ . '/../models/Document.php';
    require_once __DIR__ . '/../middleware/auth.php';
    require_once __DIR__ . '/../middleware/upload.php';
    require_once __DIR__ . '/../helpers/retrain_chatbot.php';

    $model = new AcademicProject($db);
    ...
    if ($method === 'POST') {
        ...
        if (strpos($contentType, 'multipart/form-data') !== false) {
            ...
            if (!empty($_FILES['files'])) {
                $uploaded = $uploader->handleUpload($_FILES, 'academic_projects');
                if (!empty($uploaded)) {
                    $fileInfo = $uploaded[0];
                    $iconDocId = $documentModel->create(
                        ...
                        'academic_project_icon', $fileInfo['mime_type'], $fileInfo['file_size']
                    );
        ...
        if (!empty($_FILES['detail_files'])) {
            $detailUploaded = $uploader->handleUpload($_FILES, 'academic_projects');
            ...
            foreach ($data['details'] as $idx => $detail) {
                ...
                if (isset($detailFiles[$idx])) {
                    ...
                    $detailDocId = $documentModel->create(
                        ...
                        'academic_project_detail', $fi['mime_type'], $fi['file_size']
                    );
    ...
    if ($method === 'PUT' && $id) {
        ...
        retrainChatbot($db, 'academic_projects');
        echo json_encode(["message" => "Academic project updated"]);
        return;
    }
    ...
    if ($method === 'DELETE' && $id) {
        ...
        retrainChatbot($db, 'academic_projects');
        echo json_encode(["message" => "Academic project deleted"]);
        return;
    }
