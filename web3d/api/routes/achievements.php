    ...
    if ($method === 'POST') {
        ...
        if (strpos($contentType, 'multipart/form-data') !== false) {
            ...
            if (!empty($_FILES['files'])) {
                $uploaded = $uploader->handleUpload($_FILES, 'achievements');
                if (!empty($uploaded)) {
                    $fileInfo = $uploaded[0];
                    $iconDocId = $documentModel->create(
                        ...
                        'achievement_icon', $fileInfo['mime_type'], $fileInfo['file_size']
                    );
        ...
        if (!empty($_FILES['detail_files'])) {
            $detailUploaded = $uploader->handleUpload($_FILES, 'achievements');
            ...
            foreach ($data['details'] as $idx => $detail) {
                ...
                if (isset($detailFiles[$idx])) {
                    ...
                    $detailDocId = $documentModel->create(
                        ...
                        'achievement_detail', $fi['mime_type'], $fi['file_size']
                    );
                    $detail['image_url'] = $fi['relative_path'];
                    $detail['document_id'] = $detailDocId;
                }
                $model->createDetail($achievementId, $detail);
            }
        }
