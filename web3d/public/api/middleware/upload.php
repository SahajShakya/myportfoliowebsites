    ...
    public function handleUpload($files, $basePath = '') {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($fileError !== UPLOAD_ERR_OK) {
                $errorMessages = [
                    UPLOAD_ERR_INI_SIZE => "File '$fileName' exceeds server upload limit",
                    UPLOAD_ERR_FORM_SIZE => "File '$fileName' exceeds form size limit",
                ...
                $msg = $errorMessages[$fileError] ?? "Unknown upload error (code $fileError) for '$fileName'";
                return ["error" => $msg];
            }

            if ($fileSize > $this->maxFileSize) {
                return ["error" => "File '$fileName' exceeds maximum size of " . round($this->maxFileSize / 1048576) . "MB"];
            }
