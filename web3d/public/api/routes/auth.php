...
function handleAuthRoutes($method, $segments, $db) {
    ...
    if ($method === 'GET' && $action === 'status') {
        $token = $auth->getAccessTokenFromHeader() ?? $auth->getAccessTokenFromCookie();
        $payload = $token ? $auth->verifyAccessToken($token) : null;

        $user = null;
        if ($payload) {
            $candidate = $userModel->findById($payload['user_id']);
            if ($candidate) {
                unset($candidate['password_hash']);
                $candidate['role'] = $candidate['role_name'] ?? null;
                $user = $candidate;
            }
        }
    ...
    if ($method === 'POST' && $action === 'profile-image') {
        ...
        $stmt->execute([$fileInfo['relative_path'], $payload['user_id']]);

        $stmt = $db->prepare("INSERT INTO site_settings (setting_key, bg_image_id) VALUES ('profile_image', ?) ON DUPLICATE KEY UPDATE bg_image_id = ?");
    ...
    if ($method === 'POST' && $action === 'section-bg-image') {
        ...
        $allowedKeys = ['about_bg_image', 'academics_bg_image', 'achievements_bg_image', 'journey_bg_image', 'projects_bg_image', 'testimonials_bg_image', 'contact_bg_image'];
    ...
    if ($method === 'DELETE' && $action === 'section-bg-image') {
        $payload = $auth->authenticate();
        if (!$payload) {
            http_response_code(401);
            echo json_encode(["error" => "Unauthorized"]);
        ...
        $stmt = $db->prepare("SELECT bg_image_id FROM site_settings WHERE setting_key = ?");
        $stmt->execute([$sectionKey]);
        $row = $stmt->fetch();
        if ($row && !empty($row['bg_image_id'])) {
            $doc = $docModel->findById($row['bg_image_id']);
            if ($doc) {
                $uploader->deleteFileByAbsolute($doc['absolute_path']);
                $docModel->delete($row['bg_image_id']);
            }
        }

        $clear = $db->prepare("UPDATE site_settings SET bg_image_id = NULL WHERE setting_key = ?");
        $clear->execute([$sectionKey]);

        echo json_encode(["message" => "Background image removed"]);
        return;
    }
    ...
    if ($method === 'POST' && $action === 'cvs') {
        ...
        if ($isActive) {
            $stmt = $db->prepare("INSERT INTO site_settings (setting_key, bg_image_id) VALUES ('active_cv', ?) ON DUPLICATE KEY UPDATE bg_image_id = ?");
            $stmt->execute([$docId, $docId]);
        }
    ...
    if ($method === 'PUT' && $action === 'cvs') {
        ...
        if (isset($data['is_active']) && $data['is_active'] === true) {
            ...
            $stmt = $db->prepare("INSERT INTO site_settings (setting_key, bg_image_id) VALUES ('active_cv', ?) ON DUPLICATE KEY UPDATE bg_image_id = ?");
            $stmt->execute([$existing['document_id'], $existing['document_id']]);
        } else {
    ...
    if ($method === 'DELETE' && $action === 'cvs') {
        ...
        if (!empty($existing['document_id'])) {
            ...
            $activeCvStmt = $db->prepare("SELECT bg_image_id FROM site_settings WHERE setting_key = 'active_cv'");
            $activeCvStmt->execute();
            $activeCvRow = $activeCvStmt->fetch();
            if ($activeCvRow && $activeCvRow['bg_image_id'] === $existing['document_id']) {
                $db->prepare("UPDATE site_settings SET bg_image_id = NULL WHERE setting_key = 'active_cv'")->execute();
            }
