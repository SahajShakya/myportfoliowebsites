...
class KnowledgeChunk {
    ...
    public function searchByTypes($types, $query, $limit = 5) {
        $keywords = array_filter(array_map('trim', explode(' ', $query)), function($w) {
            return strlen($w) > 2;
        });
        ...
        $stmt = $this->conn->prepare(
            "SELECT id, source_table, source_id, chunk_text, metadata, ({$scoreExpr}) AS match_score
             FROM knowledge_chunks
             WHERE JSON_UNQUOTE(JSON_EXTRACT(metadata, '$.type')) IN ({$typePlaceholders})
