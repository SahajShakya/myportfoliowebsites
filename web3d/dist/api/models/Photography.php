...
class Photography {
    ...
    public function create($data) {
        ...
        try {
            ...
            $stmt = $this->conn->prepare(
                "INSERT INTO {$this->table} (id, title, description, created_at, updated_at) 
                 VALUES (?, ?, ?, NOW(), NOW())"
            );
            $stmt->execute([
                $id,
    ...
    private function insertPhotos($photographyId, $photos) {
        $stmt = $this->conn->prepare(
            "INSERT INTO {$this->photosTable} (id, photography_id, photo_url, caption, display_order, document_id) VALUES (?, ?, ?, ?, ?, ?)"
        );
        foreach ($photos as $index => $photo) {
            ...
            if ($url) {
                $id = generateUUID();
                $stmt->execute([$id, $photographyId, $url, $caption, $photo['display_order'] ?? $index, $docId]);
            }
        }
    }
    ...
    private function getTags($photographyId) {
        $stmt = $this->conn->prepare("SELECT id, tag FROM {$this->tagsTable} WHERE entity_type = 'photography' AND entity_id = ?");
        $stmt->execute([$photographyId]);
    ...
    private function insertTags($photographyId, $tags) {
        $stmt = $this->conn->prepare(
            "INSERT INTO {$this->tagsTable} (id, entity_type, entity_id, tag) VALUES (?, 'photography', ?, ?)"
        );
        foreach ($tags as $tag) {
            ...
            if ($tagName !== '') {
                $id = generateUUID();
                $stmt->execute([$id, $photographyId, $tagName]);
            }
        }
    }

    private function deleteTags($photographyId) {
        $stmt = $this->conn->prepare("DELETE FROM {$this->tagsTable} WHERE entity_type = 'photography' AND entity_id = ?");
        $stmt->execute([$photographyId]);
    }
}
