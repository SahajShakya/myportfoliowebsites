...
class Project {
    ...
    private $tagsTable = 'project_tags';
    private $detailsTable = 'project_details';
    ...
    public function createDetail($projectId, $detail) {
        $stmt = $this->conn->prepare(
            "INSERT INTO {$this->detailsTable} (project_id, icons, image_url, image_description, document_id, contents, display_order) 
             VALUES (?, ?, ?, ?, ?, ?, ?)"
        );
        $imgUrl = '';
    ...
    private function getTags($projectId) {
        $stmt = $this->conn->prepare("SELECT tag FROM {$this->tagsTable} WHERE project_id = ?");
        $stmt->execute([$projectId]);
        return $stmt->fetchAll(PDO::FETCH_COLUMN);
    }

    private function insertTags($projectId, $tags) {
        $stmt = $this->conn->prepare("INSERT INTO {$this->tagsTable} (project_id, tag) VALUES (?, ?)");
        foreach ($tags as $tag) {
    ...
    private function deleteTags($projectId) {
        $stmt = $this->conn->prepare("DELETE FROM {$this->tagsTable} WHERE project_id = ?");
        $stmt->execute([$projectId]);
    }

    private function getDetails($projectId) {
        $stmt = $this->conn->prepare(
            "SELECT pd.id, pd.project_id, pd.icons, pd.image_url, pd.image_description, pd.document_id, pd.contents, pd.display_order,
    ...
    private function deleteDetails($projectId) {
        $stmt = $this->conn->prepare("DELETE FROM {$this->detailsTable} WHERE project_id = ?");
        $stmt->execute([$projectId]);
    }
}
