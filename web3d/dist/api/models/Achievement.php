...
class Achievement {
    ...
    private $tagsTable = 'achievement_tags';
    private $detailsTable = 'achievement_details';
    ...
    public function createDetail($achievementId, $detail) {
        $stmt = $this->conn->prepare(
            "INSERT INTO {$this->detailsTable} (achievement_id, icons, image_url, image_description, document_id, contents, display_order) 
             VALUES (?, ?, ?, ?, ?, ?, ?)"
        );
        $imgUrl = '';
    ...
    private function getTags($achievementId) {
        $stmt = $this->conn->prepare("SELECT tag FROM {$this->tagsTable} WHERE achievement_id = ?");
        $stmt->execute([$achievementId]);
        return $stmt->fetchAll(PDO::FETCH_COLUMN);
    }

    private function insertTags($achievementId, $tags) {
        $stmt = $this->conn->prepare("INSERT INTO {$this->tagsTable} (achievement_id, tag) VALUES (?, ?)");
        foreach ($tags as $tag) {
            $stmt->execute([$achievementId, $tag]);
        }
    }

    private function deleteTags($achievementId) {
        $stmt = $this->conn->prepare("DELETE FROM {$this->tagsTable} WHERE achievement_id = ?");
        $stmt->execute([$achievementId]);
    }

    private function getDetails($achievementId) {
        $stmt = $this->conn->prepare(
            "SELECT ad.id, ad.achievement_id, ad.icons, ad.image_url, ad.image_description, ad.document_id, ad.contents, ad.display_order,
                    d.relative_path, d.absolute_path
             FROM {$this->detailsTable} ad 
    ...
    private function deleteDetails($achievementId) {
        $stmt = $this->conn->prepare("DELETE FROM {$this->detailsTable} WHERE achievement_id = ?");
        $stmt->execute([$achievementId]);
    }
}
